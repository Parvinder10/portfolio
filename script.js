// ============================================================
// 3D PHOTO CARD — Mouse-tracking tilt effect
// ============================================================
(function() {
    ...
    scene.addEventListener('mousemove', (e) => {
        ...
        const rotX = -dy * 14; // max 14deg
        frame.style.transform = `perspective(900px) rotateX(${rotX}deg) rotateY(${rotY}deg) scale(1.02)`;
    });

    scene.addEventListener('mouseleave', () => {
        frame.style.transition = 'transform 0.6s cubic-bezier(0.34,1.3,0.64,1)';
...
// Initialize Particles.js & Neural Canvas
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Force show top elements in case IntersectionObserver misses the first frame
    setTimeout(() => {
        document.querySelectorAll('.hero .fade-in, .hero .fade-in-up, .hero .fade-in-right').forEach(el => {
...
const projectData = {
    1: {
        ...
        color: '#0ea5e9',
        desc: 'A fully functional digital system for automating library operations. Designed with strict OOP principles in Java, it features real-time book inventory tracking, user registration, and availability checks — all wrapped in a rich JavaSwing desktop interface.',
        tech: ['Java', 'JavaSwing', 'OOP', 'File I/O'],
        github: 'https://github.com/Parvinder10',
        live: '#',
        highlights: [
            'Modular OOP architecture with Inheritance, Encapsulation & Polymorphism',
            'Real-time book availability lookups with persistent data storage',
            'Complete user registration and authentication system',
            'Dynamic JavaSwing GUI with table views and form validation',
        ]
    },
    2: {
        title: 'EDA on Chronic Disease',
        date: 'May 2025',
        color: '#f59e0b',
        desc: 'Performed comprehensive Exploratory Data Analysis on large-scale chronic disease datasets using Python. Applied statistical techniques to uncover hidden patterns and identified the key variables influencing disease indicators for downstream predictive modeling.',
        tech: ['Python', 'Pandas', 'NumPy', 'Matplotlib', 'Seaborn', 'Jupyter'],
        github: 'https://github.com/Parvinder10',
        live: '#',
        highlights: [
            'Processed and cleaned 10,000+ row health dataset for analysis',
            'Generated 15+ visualizations including heatmaps, pairplots & box plots',
            'Identified top 5 correlated features influencing chronic disease risk',
            'Statistical summary reports used as inputs for ML prediction models',
        ]
    },
    3: {
        title: 'Automation Deadlock Detection',
        date: 'Mar 2025',
        color: '#ef4444',
        desc: 'A real-time deadlock detection engine implementing graph-based logic and Depth-First Search (DFS) algorithms to identify circular wait conditions in operating system process graphs. Includes an interactive web UI for live visualization.',
        tech: ['Python', 'DFS Algorithm', 'Graph Theory', 'HTML/CSS/JS'],
        github: 'https://github.com/Parvinder10',
        live: '#',
        highlights: [
            'Implemented cycle detection in directed process-resource graphs via DFS',
            'Interactive web-based UI to add processes, resources and detect cycles live',
            'Visualizes the wait-for graph with color-coded deadlock highlighting',
            'Handles edge cases: multiple simultaneous deadlocks and self-loops',
        ]
    },
    4: {
        ...
        date: 'Feb 2025',
        color: '#8b5cf6',
        desc: 'An advanced interactive Excel dashboard built to analyze and visualize massive Electric Vehicle datasets. Leverages PivotTables, slicers, and dynamic charts to surface trend insights and support data-driven business decisions.',
        tech: ['MS Excel', 'PivotTables', 'Power Query', 'Slicers', 'Data Analytics'],
        github: 'https://github.com/Parvinder10',
        live: '#',
        highlights: [
            'Analyzed 150,000+ EV registration records across multiple US states',
            'Built dynamic PivotTable reports with multi-field drill-down capability',
            'Designed 8 interactive chart types: bar, line, scatter, pie with slicers',
            'KPI dashboard panels showing YoY growth, brand market share, range analytics',
        ]
    }
};
...
function openProjectModal(id) {
    ...
    // Set color variable
    modalOverlay.style.setProperty('--modal-color', p.color);
    document.getElementById('modal-accent-bar').style.background = p.color;
    document.getElementById('modal-accent-bar').style.boxShadow = `0 0 20px ${p.color}`;
    document.getElementById('modal-folder-icon').style.color = p.color;
    document.getElementById('modal-folder-icon').style.textShadow = `0 0 25px ${p.color}`;
    ...
    techEl.innerHTML = p.tech.map(t => `<span class="modal-tech-pill" style="border-color:${p.color};color:${p.color}">${t}</span>`).join('');

    const hlEl = document.getElementById('modal-highlights');
    hlEl.innerHTML = p.highlights.map(h => `<li style="--modal-color:${p.color}">${h}</li>`).join('');

    // Update button links and colors
    document.getElementById('modal-github-link').href = p.github || '#';
    document.getElementById('modal-project-link').href = p.live || '#';
    document.getElementById('modal-github-link').style.background = `linear-gradient(90deg, ${p.color}, #8b5cf6)`;
    document.getElementById('modal-project-link').style.borderColor = p.color;
    document.getElementById('modal-project-link').style.color = p.color;

    // Pause Lenis so modal can scroll natively
    if (window.lenis) window.lenis.stop();

    // Scroll modal container to top on open
    ...
    // GSAP stagger animation for modal content items
    if (typeof gsap !== 'undefined') {
        gsap.fromTo(
            '.modal-header .modal-top, .modal-body .modal-desc, .modal-body .modal-section, .modal-footer',
            { y: 22, opacity: 0 },
            { y: 0, opacity: 1, duration: 0.5, stagger: 0.1, ease: 'power3.out', delay: 0.18, clearProps: 'all' }
        );
    }
}
...
// ============================================================
// MODAL PARTICLE CANVAS — 3D floating dots effect
// ============================================================
(function() {
    ...
    function resize() {
        const container = canvas.closest('.project-modal-container');
        if (!container) return;
    ...
    function draw() {
        ...
        particles.forEach(p => {
            ...
            p.y += p.dy;
            p.alpha += p.da;
            if (p.alpha <= p.alphaMin || p.alpha >= p.alphaMax) p.da *= -1;
    ...
    // Start/stop with modal
    const observer = new MutationObserver(() => {
        const overlay = document.getElementById('project-modal');
        if (!overlay) return;
        if (overlay.classList.contains('active')) {
            const p = overlay.querySelector('.project-modal-container');
            const style = getComputedStyle(p);
            // Get the accent color from the inline style
            color = p.style.getPropertyValue('--modal-color') || '#0ea5e9';
            running = true;
            resize();
            spawnParticles();
